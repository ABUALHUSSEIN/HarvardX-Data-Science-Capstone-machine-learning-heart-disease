---
title: "HarvardX PH125.9x | Data Science Heart Disease Capstone Project"
...
# Exploratory Data Analysis
Before building predictive models, we conducted an exploratory data analysis to understand 
the distribution of variables and their relationships with heart disease.
...
```{r heart-disease-distribution, fig.width=5.5, fig.height=4, dpi=300}
# Compute percentages
data_perc <- data %>% 
  count(target) %>% 
...
```{r Distribution_of_Numerical_Features, fig.width=12, fig.height=10, dpi=300, message=FALSE, warning=FALSE}
...
2. **st_depression**
    ...
    Heart disease patients (target == 1) show higher ST depression, 
    which typically reflects abnormal ECG response to stress.

3. **num_major_vessels**
   
    The count plot shows a strong difference in the number of major vessels colored by fluoroscopy.
    Higher counts (2 or 3 vessels) are much more common in patients without heart disease (target == 0),
    indicating that fewer colored vessels may signal higher risk.
...
```{r Binary_categorical_features, fig.width=10, fig.height=6, dpi=300, message=FALSE, warning=FALSE}
...
```{r Multi_categorical_features, fig.width=10, fig.height=7, dpi=300, message=FALSE, warning=FALSE}
...
To assess the strength of association between categorical variables in the dataset, 
we computed Cramer's V correlation coefficients and visualized them using a heatmap.

Cramer's V values range from 0 to 1. 
...
# ROC Curve
Now, we can compare the ROC curves for our  machine learning models.
```{r ROC_and_AUC_Comparison, fig.width=10, fig.height=7, message=FALSE, warning=FALSE}
# Get predictions
...
4. Important predictors: The most significant predictors of heart disease identified by the logistic regression model were the number of major vessels, 
  male sex, flat ST slope, typical angina chest pain type, and resting blood pressure.
...
1. [What Causes Heart Disease? Explaining the Model – Kaggle](https://www.kaggle.com/code/tentotheminus9/what-causes-heart-disease-explaining-the-model)  
2. [Tidymodels: Case Study](https://www.tidymodels.org/start/case-study/)  
3. [Microsoft Learn: Machine Learning with R](https://learn.microsoft.com/en-us/training/paths/machine-learning-with-r/)  
4. [Julia Silge – GDPR Violations & Modeling](https://juliasilge.com/blog/gdpr-violations/)  
5. [TMWR – Preprocessing Table](https://www.tmwr.org/pre-proc-table)  
6. [Heart Disease Dataset – Kaggle](https://www.kaggle.com/datasets/data855/heart-disease)  
7. [Heart Disease Dataset – UCI Machine Learning Repository](https://archive.ics.uci.edu/dataset/45/heart+disease)
